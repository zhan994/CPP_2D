...
int main()
{
  ...
  if (bg::intersects(seg1, seg2))
  {
    ...
    Bool::Intersection(seg1, seg2, res);
    for (const auto &pt : res) std::cout << "Intersection: " << String::PointToString(pt) << std::endl;
  }

  std::cout << String::SegmentToString(seg2) << " & " << String::SegmentToString(seg3)
            << "intersection state: " << bg::intersects(seg2, seg3) << std::endl;
  if (bg::intersects(seg2, seg3))
  {
    std::vector<Point> res;
    Bool::Intersection(seg2, seg3, res);
    for (const auto &pt : res) std::cout << "Intersection: " << String::PointToString(pt) << std::endl;
  }
  std::cout << String::SegmentToString(seg4) << " & " << String::SegmentToString(seg5)
            << "intersection state: " << bg::intersects(seg4, seg5) << std::endl;
  if (bg::intersects(seg4, seg5))
  {
    std::vector<Point> res;
    Bool::Intersection(seg4, seg5, res);
    for (const auto &pt : res) std::cout << "Intersection: " << String::PointToString(pt) << std::endl;
  }
  ...
  std::cout << String::SegmentToString(seg6) << " & pg1 intersection state: " << bg::intersects(seg6, pg1) << std::endl;
  if (bg::intersects(seg6, pg1))
  {
    std::vector<Point> res;
    Bool::Intersection(seg6, pg1, res);
    for (const auto &pt : res) std::cout << "Intersection: " << String::PointToString(pt) << std::endl;
  }

  Segment seg7(Point(0, 0.5), Point(0, 1));
  std::cout << String::SegmentToString(seg7) << " & pg1 intersection state: " << bg::intersects(seg7, pg1) << std::endl;
  if (bg::intersects(seg7, pg1))
  {
    std::vector<Point> res;
    Bool::Intersection(seg7, pg1, res);
    for (const auto &pt : res) std::cout << "Intersection: " << String::PointToString(pt) << std::endl;
  }

  Segment seg8(Point(-1, 1), Point(1, -1));
  std::cout << String::SegmentToString(seg8) << " & pg1 intersection state: " << bg::intersects(seg8, pg1) << std::endl;
  if (bg::intersects(seg8, pg1))
  {
    std::vector<Point> res;
    Bool::Intersection(seg8, pg1, res);
    for (const auto &pt : res) std::cout << "Intersection: " << String::PointToString(pt) << std::endl;
  }
  ...
  std::cout << String::SegmentToString(seg9) << " & pg2 intersection state: " << bg::intersects(seg9, pg2) << std::endl;
  if (bg::intersects(seg9, pg2))
  {
    std::vector<Point> res;
    Bool::Intersection(seg9, pg2, res);
    for (const auto &pt : res) std::cout << "Intersection: " << String::PointToString(pt) << std::endl;
  }
  if (0)
  {
  ...
  Bool::Difference(pg3, pg1, pg3_diff_pg1);
  ...
  Bool::Union(pg3, pg1, pg3_union_pg1);
  ...
  pg4.outer().push_back(Point(-10, -10));
  bg::correct(pg4);
  std::cout << Bool::CoveredBy(seg10, pg4) << std::endl;
  std::cout << Bool::CoveredBy(seg11, pg4) << std::endl;
  std::cout << Bool::CoveredBy(seg12, pg4) << std::endl;
  if (1)
  {
    vis_ptr->Clear();
